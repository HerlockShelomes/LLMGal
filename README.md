...
完整多模态流程依赖三类第三方服务：OpenAI 兼容的文本模型接口、火山引擎图像服务和语音合成服务。后端从环境变量读取服务配置，变量名称见根目录 `.env.example`。

使用前请完成以下工作：

1. 为三类服务准备仅用于开发或测试的凭据。
2. 将 `.env.example` 复制为 `.env`，只在本机填写真实值。
3. `.env` 已被 Git 忽略，禁止提交真实密钥。
4. 对曾经提交到仓库历史中的凭据立即作废并重新生成。
...
浏览器访问 <http://127.0.0.1:8000/>，返回 `{"status":"alive"}` 表示服务已启动。
...
```bash
./scripts/run_module1_tests.sh -q
```

首次运行会创建 Python 3.11 虚拟环境并安装精简测试依赖；后续运行可离线完成。测试包含 34 条与课程用例编号对应的 pytest 用例。文本、语音和图片依赖均使用预先写定的固定回复或固定异常 Mock/Stub，并由测试夹具阻断未 Mock 的 HTTP 请求；整个模块一测试不会调用真实大模型或第三方 API，不需要 API Key，也不会产生调用费用。结果保存到 `artifacts/test-results/`。

课程材料位于 `docs/module1/`，其中包含原始用例清单、执行结果、缺陷报告、测试报告和 AI 使用披露。GitHub Actions 使用同一条命令，配置见 `.github/workflows/module1-tests.yml`。

现场演示建议在本机运行上述脚本。单元测试不需要部署完整系统；如需展示服务存活，可另行启动后端并访问健康检查。当前完整多模态流程依赖外部服务，不适合作为现场测试的唯一方案。
...
以下内容仅供项目维护者测试真实服务，**不属于模块一，也不要在模块一演示中执行**。
...
前端目前只有一个基础冒烟测试，原有后端 WebSocket 测试仍依赖外部服务。模块一新增了 34 条隔离测试，覆盖 WebSocket 协议、嵌套配置、情绪解析、语音与图片异常、资源索引以及同角色并发状态。
...
- 原有 `backend/test_websocket.py` 仍依赖真实外部服务；课程测试位于 `backend/tests/`。
- 当前前端 `build` 与 `lint` 存在待修复问题。
- 历史提交中曾出现服务凭据；删除当前代码中的值不能替代凭据轮换。
